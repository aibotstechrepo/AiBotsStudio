...
{
    public sealed class ExcelHAlign : CodeActivity
    {
        public enum Align
        {
            Left,
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            try
            {
                if(align == 0)
                {
                    Excel_Core.H_Align_Left(from, to, er, ec);
                }
                else if(align == 1)
                {
                    Excel_Core.H_Align_Right(from, to, er, ec);
                }
                else if(align == 2)
                {
                    Excel_Core.H_Align_Center(from, to, er, ec);
                }
                else
                {
                    Excel_Core.H_Align_Justify(from, to, er, ec);
                }
            }
            catch (Exception ex)
            {
