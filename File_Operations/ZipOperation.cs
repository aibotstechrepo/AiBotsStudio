...
{
    public class ZipOperation : IDisposable
    {
        /// <summary>
        ...
        // leave the stream open after the ZipOperation object is disposed
        ...
        /// <returns>A valid ZipOperation object</returns>
        internal static ZipOperation Create(string _filename, string _comment = null)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            ZipOperation zip = Create(stream, _comment);
        ...
        /// <param name="_leaveOpen">true to leave the stream open after the ZipOperation object is disposed; otherwise, false (default).</param>
        /// <returns>A valid ZipOperation object</returns>
        internal static ZipOperation Create(Stream _stream, string _comment = null, bool _leaveOpen = false)
        {
            ZipOperation zip = new ZipOperation()
            {
        ...
        /// <param name="_leaveOpen">true to leave the stream open after the ZipOperation object is disposed; otherwise, false (default).</param>
        /// <returns>A valid ZipOperation object</returns>
        internal static ZipOperation Open(Stream _stream, FileAccess _access, bool _leaveOpen = false)
        {
        ...
        internal ZipFileEntry AddFile(Compression _method, string _pathname, string _filenameInZip, string _comment = null)
        {
        ...
        internal ZipFileEntry AddStream(Compression _method, string _filenameInZip, Stream _source, DateTime _modTime, string _comment = null)
        {
#if NOASYNC
...
#else
        internal async Task<ZipFileEntry>
#endif
        ...
        internal void AddDirectory(Compression _method, string _pathname, string _pathnameInZip, string _comment = null)
        {
            if (Access == FileAccess.Read)
        ...
        internal bool ExtractFile(ZipFileEntry _zfe, string _filename)
        {
        ...
        /// <returns>True if success, false if not.</returns>
        /// <remarks>Unique compression methods are Store and Deflate</remarks>
        internal bool ExtractFile(ZipFileEntry _zfe, out byte[] _file)
        {
        ...
        internal static bool RemoveEntries(ref ZipOperation _zip, List<ZipFileEntry> _zfes)
        {
            ...
            try
            {
                var tempZip = ZipOperation.Create(tempZipName, string.Empty);
                ...
                _zip = ZipOperation.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        */
        ...
        Store(ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                {
                    _zfe.Crc32 = ZipOperation.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                }
        ...
        */
        ...
        public static string ExtractZipFiles(string ZipFileLocation, string ExtractedFileLocation)
        {
            string fileName = ZipFileLocation;

            string ErrorReport = null;
            try
            {
                LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Function started", "N", "1101", null, null, null);
                if (!Directory.Exists(ExtractedFileLocation))
                {
                    LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Extraction location not found hence created new file", "N", "1101", null, null, null);
                    Directory.CreateDirectory(ExtractedFileLocation);
                }
                ZipOperation zip = ZipOperation.Open(ZipFileLocation, FileAccess.Read);

                List<ZipOperation.ZipFileEntry> dir = zip.ReadCentralDir();
                fileName = Path.GetFileNameWithoutExtension(zip.FileName);
                if (dir.Count > 0)
                {
                    LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains files", "N", "1112", null, null, null);
                    if (dir.Count == 1)
                    {
                        LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains 1 file", "N", "1116", null, null, null);
                        foreach (ZipOperation.ZipFileEntry entry in dir)
                        {
                            try
                            {
                                string zipFileName = Path.GetFileNameWithoutExtension(zip.FileName);
                                string FileType = null;
                                string[] tempFileType = entry.FilenameInZip.Split('.');
                                if (tempFileType.Length > 1)
                                {
                                    FileType = $"{"."}{tempFileType[tempFileType.Length - 1]}";
                                }
                                if (FileType.ToLower().Contains("mht"))
                                {
                                    LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains .mht file", "N", "1130", null, null, null);
                                    zipFileName = $"{zipFileName}{FileType}";
                                    string OutputFileName = Path.Combine(ExtractedFileLocation, zipFileName);
                                    zip.ExtractFile(entry, OutputFileName);
                                }
                                else
                                {
                                    LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains no .mht file", "N", "1137", null, null, null);

                                    ErrorReport = $" {ErrorReport }{ @"\t\u2022 "}{fileName}{" does not contails *.mht files."}{System.Environment.NewLine}";
                                }

                            }
                            catch (Exception ex)
                            {
                                LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Unable to extract zip", "Y", "1145", ex.StackTrace, ex.ToString(), "Moving next");
                                ErrorReport = $" {ErrorReport }{ "\t\u2022 "}{fileName}{" unable to extract files. Zip file maybe corrupted or unable to extract the files."}{System.Environment.NewLine}";
                    ...
                    {
                        LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains multiple file", "N", "1116", null, null, null);
                        ErrorReport = $" {ErrorReport }{ @"\t\u2022 "}{fileName}{" contails multiple files. Total files available in zip : "}{dir.Count}{System.Environment.NewLine}";
                    }

                }
                else
                {
                    LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip contains no files", "N", "1173", null, null, null);

                    ErrorReport = $" {ErrorReport }{ @"\t\u2022 "}{fileName}{" does not contails any files."}{System.Environment.NewLine}";
            ...
            {
                LogHandle.ModuleLogFile("ZipOperation", "Extract zip files", "Zip Extraction error.", "Y", "1182", ex.StackTrace, ex.ToString(), "Moving next");
                ErrorReport = $" {ErrorReport }{ @"\t\u2022 "}{fileName}{" unable to extract files. Zip file maybe corrupted or unable to extract the files."}{System.Environment.NewLine}";
            }
            return ErrorReport;
        }
